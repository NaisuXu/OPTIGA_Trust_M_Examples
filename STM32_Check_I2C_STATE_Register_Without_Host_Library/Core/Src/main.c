...
int main(void)
{
  ...
  uint8_t ADDR = 0x30;
  uint8_t Write_Data[1] = {0x82};
  uint8_t Read_Data[5] = {0};
  while (1)
  {
    while(HAL_I2C_Master_Transmit(&hi2c1, ADDR<<1, Write_Data, 1, 1000) != HAL_OK) { }
    while(HAL_I2C_Master_Receive(&hi2c1, ADDR<<1, Read_Data, 4, 1000) != HAL_OK) { }
