...
int8_t example_util_update_count(struct pt *pt)
{
	...
	static uint8_t need_initial_threshold = 0;
	...
	static uint8_t now_counter_object_data[8];
    ...
    do
    {
        ...
        bytes_to_read = sizeof(now_counter_object_data);
        optiga_lib_status = OPTIGA_LIB_BUSY;
        return_status = optiga_util_read_data(me_util, 0xE120, 0, now_counter_object_data, &bytes_to_read); // read data
        if(return_status != OPTIGA_LIB_SUCCESS)
        {
        	printf("Error: file %s line %d\n", __FILE__, __LINE__);
        }
        ...
        else
        {
        	for(int i=4; i<8; i++)
        	{
        		if(now_counter_object_data[i] != initial_counter_object_data[i])
        		{
        			need_initial_threshold = 1;
        		}
        	}
        }

        if(need_initial_threshold)
        {
            optiga_lib_status = OPTIGA_LIB_BUSY;
        ...
        {
        	uint8_t *p = now_counter_object_data;
        	printf("Counter data:");
